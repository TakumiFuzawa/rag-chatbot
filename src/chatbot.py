...
"""
    ...
    for attempt in range(3):
        try:
            ...
            if attempt < 2:
                print(f"⏳ 少し待ってリトライします...（{attempt+1}/3）")
                time.sleep(30)
            else:
                raise e
...
def chat():
    ...
    while True:
        ...
        documents = search_documents(query, embedding)

        if not documents:
            print("AI：関連する情報が見つかりませんでした。\n")
            continue

        answer = generate_answer(client, query, documents)

        print(f"\nAI：{answer}")
        print(f"\n📄 参照ページ：{[doc['page'] for doc in documents]}")
        print("-" * 50)
        print()
